...
/*
    ...
    public loadBase64(path: string, filename: string) {
        ...
        let loading = File.readAsDataURL(path, filename).then((base64string) => {
            this.images.find(img => img.path === path).base64string = base64string.toString();  
        }); 
    ...
    public getImage(path: string, filename: string): string {
        let res = this.images.find(img => img.path === path); 

        if (res !== undefined) {
            return res.base64string; 
        } else {
            this.loadBase64(path, filename); 
        }
    }
    public getImages() {
